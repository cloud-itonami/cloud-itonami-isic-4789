(ns stallmarketops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (StallMarketAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the StallMarketGovernor (:govern) and the rollout phase gate
  ...
  human market-stall coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [stallmarketops.advisor :as advisor]
            [stallmarketops.governor :as governor]
            [stallmarketops.phase :as phase]
...
(defn build
  ...
  `stallmarketops.store/Store`).
  opts:
    :advisor      -- a `stallmarketops.advisor/Advisor` (default: mock-advisor)
