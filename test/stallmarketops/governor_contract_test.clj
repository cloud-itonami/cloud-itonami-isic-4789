(ns stallmarketops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :stall-id "stall-1" :patch {:units-sold 18}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-compliance-concern :stall-id "stall-1"
                                :patch {:concern "suspected counterfeit goods" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :stall-id "stall-1"
                                :patch {:item "display fixtures" :quantity 1 :estimated-cost 2200.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest unverified-stall-hard-hold
  (testing "registered but permit-unverified stall -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
...
(deftest unverified-stall-supply-order-hard-hold
  (testing "a supply-order targeting a permit-unverified stall -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
          ctx {:actor-id "test-4b" :phase 3}
          result (exec-request actor "t4b"
                               {:op :coordinate-supply-order :stall-id "stall-3"
                                :patch {:item "general merchandise" :quantity 50 :estimated-cost 300.0}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unverified stall must HARD hold, even for a supply order"))))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into permit-issuance/dispute-resolution-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8b"
                     {:op :log-sales-record :stall-id "unknown" :patch {:units-sold 18}}
                     ctx)
